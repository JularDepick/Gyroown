...
%USERPROFILE%\.Gyroown\
  ...
  ├── preview\
  │   └── {hashID}.gyropv             ← 已实现 (pv=preview)
  ├── log\
  │   ├── error-yyyy-mm-dd-yyyy-mm-dd.txt  ← 已实现 (100KB 切片)
  │   └── run-yyyy-mm-dd-yyyy-mm-dd.txt    ← 已实现
  ├── key\
  │   └── portrait.*            ← 图片密码
  └── settings.json             ← 已实现 (主题/语言/强调色)
```
...
public interface IPasswordService
{
    ...
    Task ChangePasswordAsync(object oldCredential, object newCredential);
    void Lock();
    bool IsLocked { get; }
...
### 3.5 IDragDropService — 桩

> 文件: `Services/IDragDropService.cs`
...
> 文件: `Services/ILocalizationService.cs` (接口), `Services/StubServices.cs` (`StubLocalizationService`)
...
**实现细节**: INI 文件解析，`lang/zh-CN.ini` (默认) / `lang/en-US.ini`，运行时切换，7 个 section。
...
```
Gyroown/
├── App.xaml / App.xaml.cs          # 入口: 单实例, 路由
...
│   ├── ILocalizationService / StubServices   # INI 本地化
│   ├── ConfigService                # ★ 加密核心配置 (切片档位)
│   ├── InsuranceService             # 📋 密钥保险 API 桩
