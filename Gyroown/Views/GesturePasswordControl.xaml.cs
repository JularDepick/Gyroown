...
public sealed partial class GesturePasswordControl : UserControl, IPasswordControl
{
    ...
    private void OnPress(object s, PointerRoutedEventArgs e) { _drawing = true; _seq.Clear(); LineCanvas.Children.Clear(); ResetDots(); GestureGrid.CapturePointer(e.Pointer); Hit(e.GetCurrentPoint(GestureGrid).Position); }
    private void OnMove(object s, PointerRoutedEventArgs e) { if (_drawing) Hit(e.GetCurrentPoint(GestureGrid).Position); }
    ...
    private void Hit(Point p)
    {
        for (int i = 0; i < _dots.Length; i++)
        {
            ...
            {
                _seq.Add(i); _dots[i].Fill = (SolidColorBrush)Application.Current.Resources["AccentFillColorDefaultBrush"];
                if (_seq.Count >= 2) { GetCenter(_seq[^2], out double px, out double py); LineCanvas.Children.Add(new Line { X1 = px, Y1 = py, X2 = cx, Y2 = cy, Stroke = (SolidColorBrush)Application.Current.Resources["AccentFillColorDefaultBrush"], StrokeThickness = 3 }); }
                break;
            }
        }
    }
    ...
    private void GetCenter(int i, out double cx, out double cy)
    {
        ...
        if (d.HorizontalAlignment == HorizontalAlignment.Center) l = (GestureGrid.ActualWidth - d.Width) / 2 + d.Margin.Left - d.Margin.Right;
        else if (d.HorizontalAlignment == HorizontalAlignment.Right) l = GestureGrid.ActualWidth - d.Width - d.Margin.Right;
        if (d.VerticalAlignment == VerticalAlignment.Center) t = (GestureGrid.ActualHeight - d.Height) / 2 + d.Margin.Top - d.Margin.Bottom;
        else if (d.VerticalAlignment == VerticalAlignment.Bottom) t = GestureGrid.ActualHeight - d.Height - d.Margin.Bottom;
    ...
    public void Clear() { _seq.Clear(); _drawing = false; LineCanvas.Children.Clear(); ResetDots(); CountText.Text = Loc.Format("GestureControl", "HintMin"); }
